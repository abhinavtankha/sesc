...
struct { pthread_mutex_t bar_mutex; pthread_cond_t bar_cond; unsigned bar_teller; } barrier_rank;
                   /* for ranking process */
   
struct { pthread_mutex_t bar_mutex; pthread_cond_t bar_cond; unsigned bar_teller; } barrier_key;
...

{
   ...
   global = (struct global_memory *) our_malloc(sizeof(struct global_memory),__FILE__,__LINE__);
   key[0] = (int *) our_malloc(num_keys*sizeof(int),__FILE__,__LINE__);;
   key[1] = (int *) our_malloc(num_keys*sizeof(int),__FILE__,__LINE__);;
   key_partition = (int *) our_malloc((number_of_processors+1)*sizeof(int),__FILE__,__LINE__);;
   rank_partition = (int *) our_malloc((number_of_processors+1)*sizeof(int),__FILE__,__LINE__);;
   global->ranktime = (double *) our_malloc(number_of_processors*sizeof(double),__FILE__,__LINE__);;
   global->sorttime = (double *) our_malloc(number_of_processors*sizeof(double),__FILE__,__LINE__);;
   global->totaltime = (double *) our_malloc(number_of_processors*sizeof(double),__FILE__,__LINE__);;
   size = number_of_processors*(radix*sizeof(int)+sizeof(int *));
   rank_me = (int **) our_malloc(size,__FILE__,__LINE__);;
   ...
   for (i=0;i<number_of_processors;i++) {
     gp[i].rank_ff = (int *) our_malloc(radix*sizeof(int)+PAGE_SIZE,__FILE__,__LINE__);;
   }
...
pthread_create(&(__tid__[__threads__++]), NULL, (void*(*)(void *))(slave_sort), NULL);
pthread_mutex_unlock(&__intern__);
   ...
   {int aantal; for(aantal=number_of_processors - 1;aantal>0;aantal--) pthread_join(__tid__[--__threads__], NULL);}
...
/* Generated from ../Changed/radix.C */

...
*CacheAssoc<State, Addr_t, Energy>::findLine2Replace(Addr_t addr, bool ignoreLocked)
{
    ...
    Line **lineFreeOlder=0;
    Line **setEnd = theSet + assoc;
    bool isInvalidCacheCase = false;
    ...
    // and the oldest isLocked possible (lineFree)
    {
        ...
        while(l >= theSet) {
            ...
            if (!(*l)->isValid()){ // invalid cache lines
                lineFree = l;
                isInvalidCacheCase = true;
	    }
            else if (lineFree == 0 && !(*l)->isLocked()){ // for valid unlocked cache lines
		lineFree = l;
	    }
            else if(lineFreeOlder == 0 && !(*l)->isLocked()){
		lineFreeOlder = l;
	    }
    ...
    // increases locality
    Line *tmp;
    if(policy == NXLRU && lineFreeOlder != 0 && !isInvalidCacheCase){
        tmp = *lineFreeOlder;
    }

...
{
    int under10 = 0;
    int under10correct = 0;
    int under10incorrect = 0;

    int under100 = 0;
    int under100correct = 0;
    int under100incorrect = 0;

    int under1000 = 0;
    int under1000correct = 0;
    int under1000incorrect = 0;
    
    int over1000 = 0;
    int over1000correct = 0;
    int over1000incorrect = 0;



    map<int, statis>::iterator itr;
    for(itr = counterMap.begin(); itr != counterMap.end(); ++itr){
        int count = (itr->second).correctCount + (itr->second).incorrectCount;
        if(count < 10){
            under10correct += (itr->second).correctCount;
            under10incorrect += (itr->second).incorrectCount;
            under10++;
        }
        else if(count < 100){
            under100correct += (itr->second).correctCount;
            under100incorrect += (itr->second).incorrectCount;
            under100++;
        }
        else if(count < 1000){
            under1000correct += (itr->second).correctCount;
            under1000incorrect += (itr->second).incorrectCount;
            under1000++;
        }
        else{
            over1000correct += (itr->second).correctCount;
            over1000incorrect += (itr->second).incorrectCount;
            over1000++;
        }
    }

    // cout<<"under 10 total count:  "<<under10<<" \n \n";
    // cout<<"under 10 accuracy: "<<(float)under10correct*100/(under10correct + under10incorrect)<<"\n \n ";
    // cout<<"under 100 total count: "<< under100<< "\n";
    // cout<<"under 100 accuracy: "<<(float)under100correct*100/(under100correct + under100incorrect)<<"\n \n";
    // cout<<"under 1000 total count: "<< under1000 << "\n";
    // cout<<"under 1000 accuracy: "<< (float)under1000correct*100/(under1000correct + under1000incorrect) << "\n \n ";
    // cout<<"above 1000 total count: "<< over1000 << "\n";
    // cout<<"above 1000 accuracy: "<< (float)over1000correct*100/(over1000correct + over1000incorrect) << "\n \n";

    // cout<<"Total Counter: " << totalCount << "\n\n"; 
    // cout<<"Total Correct: " << totalCorrect << "\n\n";
    // cout<<"Total InCorrect: " << totalIncorrect << "\n\n";
    if(bpredEnergy)
        delete bpredEnergy;
...
PredType  BPNotTaken::predict(const Instruction * inst, InstID oracleID, bool doUpdate)
{
    ...
    // printf("%d\n", inst->calcNextInstID());
    if(inst->calcNextInstID() == oracleID){
        counterMap[inst->currentID()].correctCount = counterMap[inst->currentID()].correctCount + 1;
        totalCorrect++;
    }
    
    else{
        counterMap[inst->currentID()].incorrectCount = counterMap[inst->currentID()].incorrectCount + 1;
        totalIncorrect++;
    }
